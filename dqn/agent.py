...
class DQN:
    ...
    def key_press(self, k, mod):
        ...
        if k==key.LEFT:  self.a[0] = -1.0
        if k==key.RIGHT: self.a[0] = +1.0
        if k==key.UP:    self.a[1] = +1.0
        if k==key.DOWN:  self.a[2] = +0.2   # set 1.0 for wheels to block to zero rotation


    def key_release(self, k, mod):
        if k==key.LEFT  and self.a[0]==-1.0: self.a[0] = 0
        if k==key.RIGHT and self.a[0]==+1.0: self.a[0] = 0
        if k==key.UP:    self.a[1] = 0
        if k==key.DOWN:  self.a[2] = 0
    ...
    def play_human_episode(self):
        ...
        while True:
            ...
            for index,ack in enumerate(all_actions):
                if list(ack) == self.a:
                    action_idx = np.argmax(self.a)

            reward = 0
            for _ in range(self.frame_skip):
                observation, r, done, info = self.env.step(self.a)
                if self.render:
                    self.env.render()
                reward += r
                if done:
